...
const voiceSamples = [
	{
		...
		flag: '🇺🇸',
		transcript:
			'Hello, I can help you schedule an appointment with our clinic.',
		waveform: [
			10, 25, 18, 32, 40, 30, 20, 35, 42, 28, 15, 22, 30, 38, 25, 15, 28, 35,
			20, 10,
		],
		audioFiles: {
	...
	},
	{
		...
		flag: '🇪🇸',
		transcript:
			'Hola, puedo ayudarle a programar una cita con nuestra clínica.',
		waveform: [
			8, 20, 35, 24, 42, 28, 18, 30, 38, 20, 10, 28, 35, 15, 25, 40, 22, 30, 18,
			12,
		],
		audioFiles: {
...
const AudioSample = ({ sample, currentIndustry, currentlyPlaying, setCurrentlyPlaying }: AudioSampleProps) => {
	...
	useEffect(() => {
		...
		audioRef.current = new Audio(
			sample.audioFiles[currentIndustry as keyof typeof sample.audioFiles]
		);
	...
	return (
		<div className="p-5 bg-gradient-to-br from-black/40 to-black/20 backdrop-blur-lg rounded-xl mb-4 border border-white/10 hover:border-white/20 transition-all shadow-lg hover:shadow-xl">
			<div className="flex items-center mb-3">
				...
				<span className="font-medium bg-clip-text text-transparent bg-gradient-to-r from-white to-white/70">
					{sample.language}
				</span>
			</div>
				...
				<div className="flex flex-col-reverse md:grid md:grid-cols-2 gap-4 lg:gap-16 items-center">
					...
					<div className="p-8 transition-all">
						<div className="flex items-center mb-4 sm:mb-16">
							<h3 className="text-3xl sm:text-6xl lg:text-6xl text-gradient tracking-[-0.02em] font-display font-medium py-1">
								Key benefits
							</h3>
						</div>
